...
from search_app_configs import PathConfig, IntentConfigs
...
def inference_embed(data):
    ...
    model = FastText.load(path.FASTTEXT_DIR + 'fasttext_model_v1')
    encode_raw = kiwi_f.k_morphs(data)
    encode_raw = list(map(lambda x: encode_raw[x] if x < len(encode_raw) else '#', range(configs.encode_length)))
    input = np.array(
        list(map(lambda x: model[x] if x in model.wv.index2word else np.zeros(configs.vector_size, dtype=float), encode_raw)))
    return input

def preprocess_data(tokenizing):
    configs.df['intent'] = configs.df['Service_Type'].map(configs.intent_mapping)

    if tokenizing:
        count = 0
        for i in configs.df['Question']:
            configs.df.replace(i, kiwi_f.get_no_josa_token(i), regex=True, inplace=True)
            if count % 50 == 0:
    ...
    encode = []
    decode = []
    for q, i in zip(configs.df['Question'], configs.df['intent']):
        encode.append(q)
        decode.append(i)
...
def create_graph(train=True):
    ...
    x = tf.placeholder("float", shape=[None, configs.encode_length * configs.vector_size], name='x')
    y_target = tf.placeholder("float", shape=[None, configs.label_size], name='y_target')
    # tf.reshape: -1은 자동/ 차원의 갯수와 크기를 설정 
    # 마지막 차원은 1개, 이전차원 vector_size개 , 그 이전차원 encode_length개 인 4차원 행렬
    x_image = tf.reshape(x, [-1, configs.encode_length, configs.vector_size, 1], name="x_image")
    ...
    """
    pooled_outputs = []
    for i, filter_size in enumerate(configs.filter_sizes):
        ...
        with tf.name_scope("conv-maxpool-%s" % filter_size):
            # Convolution Layer
            filter_shape = [filter_size, configs.vector_size, 1, configs.num_filters]
            ...
            b_conv1 = tf.Variable(tf.constant(0.1, shape=[configs.num_filters]), name="b")
            ...
            pooled = tf.nn.max_pool(h,
                                    ksize=[1, configs.encode_length - filter_size + 1, 1, 1],
                                    strides=[1, 1, 1, 1],
                                    padding='VALID',
                                    name="pool")
    ...
    num_filters_total = configs.num_filters * len(configs.filter_sizes)
    # 3번째 axis 제거 후 합침
    ...
    W_fc1 = tf.Variable(tf.truncated_normal(shape=[num_filters_total, configs.label_size]), name="W_fc1")
    #W_fc1 = tf.get_variable(
     #   "W_fc1",
      #  shape=[num_filters_total, label_size],
       # initializer=tf.contrib.layers.xavier_initializer())
    b_fc1 = tf.Variable(tf.constant(0.1, shape=[configs.label_size]), name="b")
    ...
    train_step = tf.train.AdamOptimizer(configs.learning_rate).minimize(cross_entropy)
...
def predict(test_data):
    try:
        ...
        dir = os.listdir(path.model_path)
        num_ckpt = 0
        for i in dir:
            try:
        ...
        # 학습된 값들을 불러옴 restore
        saver.restore(sess, path.model_path + 'check_point-' + str(num_ckpt) + '.ckpt')
...
def get_intent(text):
    ...
    if prediction is None:
        ...
        for mapping, num in configs.intent_mapping.items():
            if int(prediction) == num:
                return mapping

def preprocess(speech) ->str:
    # speech = fix(speech)
    speech = kiwi_f.get_no_josa_token(speech)
    # speech = fix(speech)
    return speech
...
if __name__ == "__main__":
    ...
    # print(configs.intent_mapping)
    # # --- word embedding 할때 ----
    # train_data_list = preprocess_data(tokenizing=True)
    # print(train_data_list)
    # train_vector_model(train_data_list, train=True)
