...
kiwi_f = kiwi_dictionary_n_fuction(pathConfigs.DICTIONARY_PATH)
...
def train_vector_model(datas, train):
    ...
    if train:
        ...
        model.save(path + 'fasttext_model_v1')
        print("TRAIN COMPLETE")
        return model
    else:
        print("LOAD SAVED MODEL")
        return FastText.load(path + 'fasttext_model_v1')
...
def embed(data):
    ...
    for encode_raw in data['encode']:
        encode_raw = kiwi_f.k_morphs(encode_raw)
...
def train_intent():
    ...
    with tf.Session() as sess:
        ...
        if len(os.listdir(path)) > 3:
            ...
            restorer.restore(sess, pathConfigs.model_path + 'check_point-' + str(num_ckpt) + '.ckpt')
        else: # 학습된거 없으면 새로 만듦
